function [ best_sol, time ] = GLbestIW_pso( func, dimension, particle_num, iter_max, lower_bound, upper_bound )
    ...
    for i = 1 : iter_max
        ...
        w = 1.1 - Best( 1, 1 : dimension - 1 ) / pBest( j, : );
        for j = i : particle_num
            Velocity( j, : ) = w * K * ( Velocity( j, : ) + rand * ( pBest( j, : )...
                - Solution( j, : ) ) + rand * ( Best( 1, 1 : dimension - 1 ) - Solution( j, : ) ) );
